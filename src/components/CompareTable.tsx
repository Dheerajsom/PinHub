...
export function CompareTable({ boards }: { boards: Board[] }) {
  ...
  const visibleRows = differencesOnly ? rows.filter((row) => uniqueValues(row.values) > 1) : rows;
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-zinc-400" role="status" aria-live="polite">{differencesOnly ? `${visibleRows.length} differing attributes shown` : `${rows.length} attributes shown`}</p>
